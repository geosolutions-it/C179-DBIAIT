...
from app.scheduler.models import GeoPackage, Task, CSVFileUploaded
from app.scheduler.utils import Schema, TaskType, TaskStatus
from django.db import connection

from .base_task import BaseTask, trace_it, time_it
    ...
    def pre_send(
        ...
        if len(colliding_tasks) > 0:
            raise exceptions.QueuingCriteriaViolated(
                "Ci sono dei task al momento in secuzione, che impediscono l'avvio del processo di import di una nuova tabella. "
        ...
        csv_file, created = CSVFileUploaded.objects.get_or_create(name=csv_file_name, ingestion_time=time_it())
        csv_file.task = current_task
        csv_file.save()
